%% Function to simulate the metapopulation model for different immune, phage, and mucin conditions
% 1. We generate the metapopulation network
% 2. We assign values to parameters
% 3. We simulate the metapopulation model
% 4. We return the population dynamics of B_S, B_R, P and I and time vector
...
%         neutrophils
%         (5) b_dist - bacterial inoculum network distribution
%         (6) p_dis - phage dose network distribution, (7) simu_time - simulation time
%         (8) mucin_conc - mucin concentration
...
function [time, res, p] = simu_metapop_mucin(B, P, I, max_neutrophil_num, b_dist, p_dist, simu_time, mucin_conc)
...
B_S0(b_dist) = B/sum(p.nodes_pergen(b_dist).*branch_volume(b_dist)); % cfu/ml
...
P2(p_dist) = P/sum(p.nodes_pergen(p_dist).*branch_volume(p_dist)); % pfu/ml
...
disp(['Total bacteria (CFU) after ' num2str(p.T) 'h: ' num2str(sum(Btot))])
