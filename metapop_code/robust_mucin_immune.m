%% Function to simulate the metapopulation model given changes in mucin and immune levels
% we further test 84 different distributions of phage dose and bacterial
% inoculum in the network

% 1. We generate the metapopulation network
% 2. We assign values to parameters
% 3. We simulate the metapopulation model
% 4. We return the total CFU after evaluating 84 different initial
% conditions
...
%         (6) mucin_conc - mucin concentration
%         (7) inocuDist_params - matrix of size 84 x 4 with bacteria and phage network
%         distribution parameters

% output: (1)Total_CFU_params - vector of size 84 x 1, with total cfu after 250 h after simulating 84 diff. initial conditions
%         (2)Time_extinct_vec - vector of size 84 x 1, with the time to infection resolution after simulating 84 diff. initial conditions
%         (3)p - structure variable with parameter values

function [Total_CFU_params, Time_extinct_vec, p] = robust_mucin_immune(B, P, I, max_neutrophil_num, simu_time, mucin_conc, inocuDist_params)
...
mucin_level = mucin_conc; % mucin concentration
...
I0(1:end) = I/sum(p.nodes_pergen.*branch_volume); % cells/ml

options = odeset('Events', @myEventsFcn, 'RelTol', 1e-6, 'AbsTol', 1e-9, 'MaxStep', 0.1);

Total_CFU_params = zeros(size(inocuDist_params, 1), 1);
Time_extinct_vec = zeros(size(inocuDist_params, 1), 1);

% Test 84 different distributions of phage dose and bacterial inoculum
for ino = 1:size(inocuDist_params, 1)
    
    % generate phage and bacterial inoculum based on distribution
    % parameters
    params_dist = inocuDist_params(ino, :);
    [bac_inocu, phage_inocu] = inoculum_distribution_fun(params_dist, p, branch_volume, B, P);
    
    % Initial bacterial density
    B_S0(1:end) = bac_inocu; % bacterial inoculum, cfu/ml
    ...
    P2(1:end) = phage_inocu; % phage dose, pfu/ml

    tspan = 2:p.T;

    yi = [BS2; BR2; P2; I2];

    [t3, y3, te, ye, ie] = ode45(@(t,y) ode_metapopchain(t,y,p), tspan, yi, options);
    ...
    if currentTime < p.T-1 % Bacterial pop died before end of simulation
        ...
        while currentTime < p.T-1
            ...
            % simulating diff eq
            [t4, y4, te, ye, ie] = ode45(@(t,y) ode_metapopchain(t,y,p), tspan3, yii, options);
            y_pre = y4;
    ...
    BS_final = res(end, 1:p.NP)'; % density
    BR_final = res(end, p.NP+1:2*p.NP)'; % density
    num_bs = BS_final.*branch_volume; % numbers
    num_br = BR_final.*branch_volume; % numbers
    extinct_bs = num_bs < 1;
    extinct_br = num_br < 1;
    ...
    Btot = Btot.*branch_volume.*p.nodes_pergen;
    Total_CFU_params(ino) = sum(Btot);
    Time_extinct_vec(ino) = p.T;
