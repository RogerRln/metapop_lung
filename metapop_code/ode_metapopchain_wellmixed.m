function dx = ode_metapopchain_wellmixed(t,y0,p)
...
% function to calculating neutrophil hopping rates
...
dI = ((a*I).*(1-(I.*(1/Ki_patch)))).*(Btot./(Btot + Kn)).*(1-((I.*sum(PA.*p.nodes_pergen))/p.max_neutrophils)) - (Tau_n_out.*I) + (Tau_n_in.*p.metapop_volume)*I; % equation for single node, system-level saturation
