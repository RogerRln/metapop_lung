...
node_id = 3; % Obtain phage and bacterial densities from node 3
time_id = 41; % Time index to retrieve phage and bacteria densities, ~25.7 hours

% calculate phage lysis rate, immune killing rate, and the combination of
% both rates
phage_killing = (p.phi*(ph(time_id, node_id))^p.g);
immune_killing = (p.ep*p.Ki)/(1+ (bs(time_id, node_id))/p.Kd);
...
legend('Simulations', 'Phage lysis', 'Immune killing', 'Combination', 'location', 'best')
legend box off
