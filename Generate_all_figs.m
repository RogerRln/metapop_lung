...
% Fig. 6 Infection clearance time difference between node i and the last node of the network as a function of the natural log of the volume ratio, V_i/V_bottom

run([pwd '/../metapop_code/Fig6_volume_clearance_time_diff.m'])
...
% Fig. 7 In vivo P.a. murine pneumonia data

run([pwd '/../image_analysis/code/Fig7_Biolumi_time_series_clearance.m'])
filename = [pwd '/../figures/Fig7.png'];
exportgraphics(figure(1), filename, 'Resolution', 300)
filename = [pwd '/../figures/Fig7.eps'];
exportgraphics(figure(1), filename)
clear
close all

% Fig. 8 Time series of total intensity signal and the infection clearance analysis using in vivo P.a. murine pneumonia data

% Fig. 8a
run([pwd '/../image_analysis/code/Fig8a_plot_tot_intensity_WT_Rag2_phage_mice.m'])
filename = [pwd '/../figures/Fig8a.png'];
exportgraphics(figure(1), filename, 'Resolution', 300)
filename = [pwd '/../figures/Fig8a.eps'];
exportgraphics(figure(1), filename)
clear
close all

% Fig. 8b
run([pwd '/../image_analysis/code/Fg8b_plot_time_to_clearance.m'])
filename = [pwd '/../figures/Fig8b.png'];
exportgraphics(figure(1), filename, 'Resolution', 300)
filename = [pwd '/../figures/Fig8b.eps'];
exportgraphics(figure(1), filename)
clear
close all
...
% Fig. S3 Robustness analysis outcome: the probability of therapeutic success given intermediate innate immune states and varying mucin levels

run([pwd '/../metapop_code/FigS3_heatmap_mucin_nlung.m'])
...
% Fig. S4 Bacterial dynamics of a well-mixed model given intermediate innate immune response levels

run([pwd '/../metapop_code/FigS4_well_mixed_immunodeficiency.m'])
