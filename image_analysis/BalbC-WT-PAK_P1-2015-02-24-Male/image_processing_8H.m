...
mice_num = regexp(mice_number, 'B.+?\-\s(\d).(\d).(\d).(\d)', 'tokens');
...
roi_w = 89;
roi_h = 100;
roi1_pos = [106-(roi_w/2), 39, roi_w, roi_h];
roi2_pos = [186-(roi_w/2), 39, roi_w, roi_h];
roi3_pos = [266-(roi_w/2), 39, roi_w, roi_h];
...
uppp_thresh = 9.9e2;%double(mean(sort_pixel_intens(end-20:end)));
