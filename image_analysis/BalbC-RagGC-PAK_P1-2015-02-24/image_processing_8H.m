...
subfolder_struct = dir([mainFolder_path '/' subFolders{3}]);
...
roi_w = 89;
roi_h = 110;
roi1_pos = [104-(roi_w/2), 34, roi_w, roi_h];
roi2_pos = [181-(roi_w/2), 34, roi_w, roi_h];
roi3_pos = [266-(roi_w/2), 34, roi_w, roi_h];
roi4_pos = [349-(roi_w/2), 34, roi_w, roi_h];
roi5_pos = [427-(roi_w/2), 34, roi_w, roi_h];
...
border = [(99-34+1) (99-34+1) (100-34+1) (102-34+1) (112-34+1)].*scale;
...
sort_pixel_intens = sort_pixel_intens(~(sort_pixel_intens > 3.5e3));
% intensity thresholds
uppp_thresh = 3.8e3;%double(mean(sort_pixel_intens(end-5:end)));
...
mice_data_8H = zeros(size(Icrop(1).mat, 1),  size(Icrop(1).mat, 2), tot_mice);

for n = 1:tot_mice
    mice_data_8H(:,:, n) = Icrop(n).mat;
end
save mice_data_8H.mat mice_data_8H

border_8H = border./2;
save border_8H.mat border_8H