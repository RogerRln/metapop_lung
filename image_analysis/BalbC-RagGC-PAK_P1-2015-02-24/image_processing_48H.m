...
roi_w = 89;
roi_h = 110;
roi1_pos = [100-(roi_w/2), 80, roi_w, roi_h];
roi2_pos = [179-(roi_w/2), 80, roi_w, roi_h];
roi3_pos = [264-(roi_w/2), 80, roi_w, roi_h];
roi4_pos = [429-(roi_w/2), 80, roi_w, roi_h];
...
border = [(144-80+1) (144-80+1) (141-80+1) (143-80+1)].*scale;
...
sort_pixel_intens = sort_pixel_intens(~(sort_pixel_intens > 4e4));
% intensity thresholds
uppp_thresh = 3e4;%double(mean(sort_pixel_intens(end-5:end)));
...
% Save data
tot_mice = 4;
mice_data_48H = zeros(size(Icrop(1).mat, 1),  size(Icrop(1).mat, 2), tot_mice);

for n = 1:tot_mice
    mice_data_48H(:,:, n) = Icrop(n).mat;
end
save mice_data_48H.mat mice_data_48H

border_48H = border./2;
save border_48H.mat border_48H

% survivors
survival_48h = [1 2 4];
save survival_48h.mat survival_48h