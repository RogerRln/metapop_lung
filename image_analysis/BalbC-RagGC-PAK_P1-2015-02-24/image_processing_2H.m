...
for k = 1:length(folder_struct)
    if (folder_struct(k).name(1) == 'N') && (folder_struct(k).name(end) == 'H')
...
subfolder_struct = dir([mainFolder_path '/' subFolders{1}]);
...
mice_num = regexp(mice_number, 'R.+?(\d).(\d).(\d).(\d)', 'tokens');
...
roi_w = 89;
roi_h = 100;
roi1_pos = [95-(roi_w/2), 47, roi_w, roi_h];
roi2_pos = [180-(roi_w/2), 47, roi_w, roi_h];
roi3_pos = [261-(roi_w/2), 47, roi_w, roi_h];
roi4_pos = [343-(roi_w/2), 47, roi_w, roi_h];
roi5_pos = [424-(roi_w/2), 47, roi_w, roi_h];
...
roi4 = drawrectangle('Position', roi4_pos, 'color', 'r');
roi5 = drawrectangle('Position', roi5_pos, 'color', 'r');
close(tiff1);
...
border = [(111-47+1) (111-47+1) (111-47+1) (121-47+1) (127-47+1)].*scale;
...
sort_pixel_intens = sort_pixel_intens(~(sort_pixel_intens > 1.4e3));
% intensity thresholds
uppp_thresh = double(mean(sort_pixel_intens(end-10:end)));
low_thresh = avg_intensity+15;
...
roi4 = drawrectangle('Position', roi4_pos.*scale, 'color', 'r');
roi5 = drawrectangle('Position', roi5_pos.*scale, 'color', 'r');
...
roi_vec = [roi1_pos.*scale; roi2_pos.*scale; roi3_pos.*scale; roi4_pos.*scale; roi5_pos.*scale];
...
for n = 1:length(Icrop)
   ...
   hold on
   plot([1 size(resize_img, 2)], [border(n)./2 border(n)./2], '-r', 'linewidth', 1.5)
   hold off
   colormap(jet)
end

% Save data
tot_mice = str2num(mice_num{1}{4}) + 1;
mice_data_2H = zeros(size(Icrop(1).mat, 1),  size(Icrop(1).mat, 2), tot_mice);

for n = 1:tot_mice
    mice_data_2H(:,:, n) = Icrop(n).mat;
end
save mice_data_2H.mat mice_data_2H

border_2H = border./2;
save border_2H.mat border_2H