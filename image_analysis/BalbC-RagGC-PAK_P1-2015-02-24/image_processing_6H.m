...
subfolder_struct = dir([mainFolder_path '/' subFolders{2}]);
...
roi_w = 89;
roi_h = 110;
roi1_pos = [104-(roi_w/2), 47, roi_w, roi_h];
roi2_pos = [183-(roi_w/2), 47, roi_w, roi_h];
roi3_pos = [267-(roi_w/2), 47, roi_w, roi_h];
roi4_pos = [350-(roi_w/2), 47, roi_w, roi_h];
roi5_pos = [431-(roi_w/2), 47, roi_w, roi_h];
...
border = [(107-47+1) (107-47+1) (108-47+1) (116-47+1) (122-47+1)].*scale;
...
sort_pixel_intens = sort_pixel_intens(~(sort_pixel_intens > 1.3e3));
% intensity thresholds
uppp_thresh = double(mean(sort_pixel_intens(end-5:end)));
...
mice_data_6H = zeros(size(Icrop(1).mat, 1),  size(Icrop(1).mat, 2), tot_mice);

for n = 1:tot_mice
    mice_data_6H(:,:, n) = Icrop(n).mat;
end
save mice_data_6H.mat mice_data_6H

border_6H = border./2;
save border_6H.mat border_6H