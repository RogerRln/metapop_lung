...
subfolder_struct = dir([mainFolder_path '/' subFolders{6}]);
...
roi_w = 89;
roi_h = 110;
roi1_pos = [94-(roi_w/2), 76, roi_w, roi_h];
roi2_pos = [179-(roi_w/2), 76, roi_w, roi_h];
roi3_pos = [264-(roi_w/2), 76, roi_w, roi_h];
roi4_pos = [425-(roi_w/2), 76, roi_w, roi_h];
%roi5_pos = [425-(roi_w/2), 35, roi_w, roi_h];
...
border = [(139-76+1) (139-76+1) (139-76+1) (147-76+1)].*scale;
...
sort_pixel_intens = sort_pixel_intens(~(sort_pixel_intens > 1.5e4));
% intensity thresholds
uppp_thresh = 1e3;%double(mean(sort_pixel_intens(end-5:end)));
...
roi_vec = [roi1_pos.*scale; roi2_pos.*scale; roi3_pos.*scale; roi4_pos.*scale];
...
% Save data
tot_mice = 4;
mice_data_72H = zeros(size(Icrop(1).mat, 1),  size(Icrop(1).mat, 2), tot_mice);

for n = 1:tot_mice
    mice_data_72H(:,:, n) = Icrop(n).mat;
end
save mice_data_72H.mat mice_data_72H

border_72H = border./2;
save border_72H.mat border_72H

% survivors
survival_72h = [1 2 4];
save survival_72h.mat survival_72h