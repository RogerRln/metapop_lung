...
for k = 1:length(folder_struct)
    if (folder_struct(k).name(1) == 'R') && (folder_struct(k).name(end) == 'H')
...
roi_w = 89;
roi_h = 110;
roi1_pos = [99-(roi_w/2), 42, roi_w, roi_h];
roi2_pos = [180-(roi_w/2), 42, roi_w, roi_h];
roi3_pos = [262-(roi_w/2), 42, roi_w, roi_h];
roi4_pos = [344-(roi_w/2), 42, roi_w, roi_h];
roi5_pos = [422-(roi_w/2), 42, roi_w, roi_h];
...
border = [(105-42+1) (105-42+1) (104-42+1) (110-42+1) (125-42+1)].*scale;
...
uppp_thresh = 1.4e3;%double(mean(sort_pixel_intens(end-3:end)));
...
mice_data_4H = zeros(size(Icrop(1).mat, 1),  size(Icrop(1).mat, 2), tot_mice);

for n = 1:tot_mice
    mice_data_4H(:,:, n) = Icrop(n).mat;
end
save mice_data_4H.mat mice_data_4H

border_4H = border./2;
save border_4H.mat border_4H