...
for k = 1:length(folder_struct)
    if (folder_struct(k).name(1) == 'M') && (folder_struct(k).name(end) == 'H')
...
exp_group = regexp(user_metadata, 'Group:\s+(.+?)\r', 'tokens');
disp(exp_group{1}{1})
mice_number = exp_group{1}{1};
mice_num = regexp(mice_number, '[rR].+?(\d).(\d).(\d)', 'tokens');
...
roi_w = 89;
roi_h = 125;
roi1_pos = [58-(roi_w/2), 199, roi_w, roi_h];
roi2_pos = [160-(roi_w/2), 199, roi_w, roi_h];
roi3_pos = [248-(roi_w/2), 199, roi_w, roi_h];
roi4_pos = [343-(roi_w/2), 199, roi_w, roi_h];
...
border = [(272-199+1) (272-199+1) (277-199+1) (277-199+1)].*scale;
...
uppp_thresh = 850;%double(mean(sort_pixel_intens(end-4:end)));
low_thresh = avg_intensity+20;
