...
mice_num = regexp(mice_number, 'r.+?(\d).(\d).(\d)', 'tokens');
...
roi_w = 89;
roi_h = 120;
roi1_pos = [54-(roi_w/2), 160, roi_w, roi_h];
roi2_pos = [150-(roi_w/2), 160, roi_w, roi_h];
roi3_pos = [246-(roi_w/2), 160, roi_w, roi_h];
roi4_pos = [339-(roi_w/2), 160, roi_w, roi_h];
...
uppp_thresh = 1e3;%double(mean(sort_pixel_intens(end-4:end)));
...
% Save data
tot_mice = str2num(mice_num{1}{3}) + 1;
